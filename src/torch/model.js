...
const torch = require("@arition/torch-js");
...
export default class Model extends Function {
  ...
  predict = async (loader) => {
      if(!(loader instanceof Dataloader)) throw new Error("You can only call a model object with an instance of the torch.utils.data.DataLoader class!");
      const promisedResults = this._makePredictions(loader);      
      const results = await Promise.all(promisedResults.map(Promise.all.bind(Promise)))
